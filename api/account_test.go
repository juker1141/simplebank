...
func TestListAccountsAPI(t *testing.T) {
	...
	testCases := []struct{
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					ListAccounts(gomock.Any(), gomock.Eq(createListAccountsArg(arg.PageID, arg.PageSize))).
					Times(1).
					Return(accounts, nil)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
...
func TestUpdateAccountAPI(t *testing.T) {
	...
	worngArg := db.UpdateAccountParams{
		ID: account.ID,
	}
	...
	testCases := []struct{
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					UpdateAccount(gomock.Any(), gomock.Eq(worngArg)).
					Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			url := fmt.Sprintf("/accounts/%d", tc.arg.ID)
...
func createListAccountsArg(pageID int32, pageSize int32) db.ListAccountsParams {
